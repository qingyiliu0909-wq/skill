...
本目录从项目的 `.skill/` 权威源中复制通用 Skill，原目录保持不变。

技能包目录统一采用 **`中文用途（原 Skill 名）`** 命名：中文部分方便在资源管理器中快速识别作用，括号里的英文保留原技能标识，便于搜索、安装和与 `SKILL.md` 中的 `name` 对照。例如：`测试用例评审（testcase-reviewer）`。
