...
set "SOURCE_DIR=%~dp0"
set "DEPENDENCIES_FILE=%SOURCE_DIR%skill-dependencies.txt"
for %%I in ("%SOURCE_DIR:~0,-1%") do set "SKILL_CATEGORY=%%~nxI"

echo ========================================
echo   Install Skills to Codex: %SKILL_CATEGORY%
echo ========================================
echo.
...
for /f "tokens=1,2 delims==" %%A in ('findstr "CODEX_SKILLS_PATH" "%SKILL_DIR%\paths.md"') do (
    set "TARGET_DIR=%PROJECT_ROOT%\%%B"
)

if not defined TARGET_DIR (
    echo ERROR: CODEX_SKILLS_PATH not found in paths.md
    exit /b 1
)
...
if exist "%DEPENDENCIES_FILE%" (
    echo.
    echo [INFO] Dependencies file found: %DEPENDENCIES_FILE%
    echo [INFO] Codex template does not install dependencies automatically yet.
)
